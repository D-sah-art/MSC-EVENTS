...
app.put('/api/events/:id', async (req, res) => {
  try {
    ...
    const updated = { ...events[index], ...req.body, id: events[index].id };
...
app.delete('/api/events/:id', async (req, res) => {
  try {
    ...
    // Also remove participants registered for this event
    const remainingParticipants = participants.filter(p => p.eventId !== removed.id);

    const [eventsSuccess, participantsSuccess] = await Promise.all([
      writeJsonFile(EVENTS_FILE, events),
      writeJsonFile(PARTICIPANTS_FILE, remainingParticipants)
    ]);

    if (eventsSuccess && participantsSuccess) {
